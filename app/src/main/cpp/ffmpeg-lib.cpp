...
Java_com_itek_facerecognize_RtmpPlayer_nativePrepare(JNIEnv *env, jobject, jstring url) {
    // 初始化
...
Java_com_itek_facerecognize_RtmpPlayer_nativeStop(JNIEnv *env, jobject) {
    //停止播放
    stop = true;
    if (frameCallback == NULL) {
        return;
...
extern "C"
JNIEXPORT void JNICALL
Java_com_itek_facerecognize_RtmpPlayer_nativeSetCallback(JNIEnv *env, jobject,
                                                           jobject callback) {
    //设置回调
    if (frameCallback != NULL) {
...
extern "C"
JNIEXPORT void JNICALL
Java_com_itek_facerecognize_RtmpPlayer_nativeStart(JNIEnv *env, jobject) {
    //开始播放
    stop = false;
    if (frameCallback == NULL) {
        return;
