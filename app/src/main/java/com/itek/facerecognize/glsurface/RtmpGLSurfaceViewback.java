package com.itek.facerecognize.glsurface;
...
public class RtmpGLSurfaceViewback extends GLSurfaceView {
    ...
    public void init(boolean isMirror, int rotateDegree, int frameWidth, int frameHeight) {
        ...
        this.rotateDegree = rotateDegree;
        this.isMirror = isMirror;
        yArray = new byte[this.frameWidth * this.frameHeight * 3];
        uArray = new byte[this.frameWidth * this.frameHeight];
        vArray = new byte[this.frameWidth * this.frameHeight];
//        uArray = new byte[this.frameWidth * this.frameHeight / 4];
//        vArray = new byte[this.frameWidth * this.frameHeight / 4];

        int yFrameSize = this.frameHeight * this.frameWidth * 3;
//        int uvFrameSize = yFrameSize >> 2;
        yBuf = ByteBuffer.allocateDirect(yFrameSize);
        yBuf.order(ByteOrder.nativeOrder()).position(0);

        uBuf = ByteBuffer.allocateDirect(yFrameSize);
        uBuf.order(ByteOrder.nativeOrder()).position(0);

        vBuf = ByteBuffer.allocateDirect(yFrameSize);
    ...
    public class YUVRenderer implements Renderer {
        private void initRenderer() {
            ...
            createTexture(frameWidth, frameHeight, GLES20.GL_LUMINANCE, uTexture);
            createTexture(frameWidth, frameHeight, GLES20.GL_LUMINANCE, vTexture);
...
//            createTexture(frameWidth / 2, frameHeight / 2, GLES20.GL_LUMINANCE, vTexture);
    ...
    private void putBGR(byte[] src, int width, int height) {
        ...
        for (int i = 0, j = 0; j < src.length; i++, j++) {
            yArray[i] = src[j];
//            uArray[i] = src[j + src.length / 3];
//            vArray[i] = src[j + src.length * 2 / 3];
        }
        yBuf.put(yArray).position(0);
        uBuf.put(uArray).position(0);
        vBuf.put(vArray).position(0);
